...
Now on **`main`** (also available on branch `redesign-v3`).
...
## Note
The previous app under `src/` remains intact. This redesign lives alongside it under `client/` and `server/` so you can migrate gradually or replace when ready.
