...
/**
 * The site guide — a character who explains the studio to first-time visitors.
 *
 ...
 *  · Captions are on by default — most people watch muted the first time.
...
export default function SiteGuide({
  ...
  useEffect(() => {
    ...
    const probe = async () => {
      try {
        ...
        /* missing or offline — stay hidden */
      }
    };
    probe();
    return () => {
  ...
  return (
    <>
      {/* Invitation pill — above the WhatsApp button, never on top of it */}
      <AnimatePresence>
        {!open && (
          <m.button
            key="guide-pill"
            ...
            aria-label={`Play the ${name} — a short tour of the site`}
          >
