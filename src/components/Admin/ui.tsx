...
export const CardHeader: React.FC<{ title: string; subtitle?: string; action?: React.ReactNode }> = ({ title, subtitle, action }) => (
  <div className="flex items-center justify-between gap-4 border-b border-[#222226] px-6 py-4">
    <div>
      <h3 className="text-sm font-bold uppercase tracking-[0.15em] text-[#EDEDED]">{title}</h3>
      {subtitle ? <p className="mt-1 text-xs text-[#888891]">{subtitle}</p> : null}
    </div>
    {action}
  </div>
);
...
export const StatCard: React.FC<{
  ...
  const toneColor = deltaTone === 'up' ? 'text-emerald-400' : deltaTone === 'down' ? 'text-red-400' : 'text-[#888891]';
  return (
    <Card className="p-5">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#888891]">{label}</p>
          <p className="mt-2 text-2xl font-black text-[#EDEDED]">{value}</p>
...
export const StatusBadge: React.FC<{ status: string }> = ({ status }) => (
  <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${statusStyles[status] || 'bg-[#888891]/10 text-[#888891]'}`}>
...
export const EmptyState: React.FC<{ message: string }> = ({ message }) => (
  <div className="flex flex-col items-center justify-center py-16 text-center">
    <p className="text-sm text-[#888891]">{message}</p>
  </div>
);
...
export const PrimaryButton: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement>> = ({ className = '', children, ...props }) => (
  <button
    {...props}
    className={`flex items-center gap-2 rounded-lg bg-[#D4AF37] px-4 py-2 text-[11px] font-bold uppercase tracking-[0.15em] text-[#0A0A0B] transition-colors hover:bg-white disabled:cursor-not-allowed disabled:opacity-60 ${className}`}
  >
    {children}
  </button>
);
...
export const GhostButton: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement>> = ({ className = '', children, ...props }) => (
  <button
    {...props}
    className={`flex items-center gap-2 rounded-lg border border-[#222226] px-4 py-2 text-[11px] font-bold uppercase tracking-[0.15em] text-[#EDEDED] transition-colors hover:border-[#D4AF37]/50 hover:text-[#D4AF37] disabled:cursor-not-allowed disabled:opacity-60 ${className}`}
  >
    {children}
  </button>
);
...
export const Input = React.forwardRef<HTMLInputElement, React.InputHTMLAttributes<HTMLInputElement>>(
  ({ className = '', ...props }, ref) => (
    <input
      {...props}
      ref={ref}
      className={`w-full rounded-lg border border-[#222226] bg-[#0A0A0B] px-3 py-2.5 text-sm text-[#EDEDED] transition-colors focus:border-[#D4AF37] focus:outline-none ${className}`}
    />
  )
);
