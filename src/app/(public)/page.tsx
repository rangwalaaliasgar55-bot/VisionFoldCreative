...
import { Counter, Reel3D, Reveal, Stars, Tilt } from "@/components/Fx";
import {
  ArrowRight,
  Clapperboard,
  Film,
  ...
  return (
    <>
        ...
        <div className="relative mx-auto max-w-7xl px-5 text-center sm:px-8">
          <Reveal>
            <div className="glass mx-auto inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-medium text-cyan-300">
              ...
              Premium video editing studio — Burbank, CA
            </div>
          </Reveal>
          <Reveal delay={100}>
          ...
          <Reveal delay={200}>
            <p className="mx-auto mt-6 max-w-2xl text-base leading-relaxed text-slate-400 sm:text-lg">{settings.heroSubtitle}</p>
          </Reveal>
          <Reveal delay={300}>
            <div className="mt-9 flex flex-wrap items-center justify-center gap-4">
              <Link href="/contact" className="group flex items-center gap-2 rounded-full bg-[#7357FF] px-7 py-3.5 text-sm font-black uppercase tracking-wider text-white shadow-lg shadow-[#7357FF]/35 transition-transform hover:scale-105 hover:bg-[#6346E8]">
                {settings.heroCta}
                <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
              </Link>
              <Link href="/work" className="glass flex items-center gap-2 rounded-full px-7 py-3.5 text-sm font-semibold text-slate-200 transition-all hover:border-white/30 hover:text-white">
          ...
          <Reveal delay={400}>
            <div className="mx-auto mt-14 grid max-w-3xl grid-cols-2 gap-4 sm:grid-cols-4">
              {[
                { value: Number(settings.statsYears), suffix: "+", label: "Years editing" },
                { value: Number(settings.statsProjects), suffix: "+", label: "Projects delivered" },
                { value: Number(settings.statsClients), suffix: "+", label: "Happy clients" },
                { value: Number(settings.statsAwards), suffix: "", label: "Awards & nods" },
              ].map((s) => (
          ...
          <div className="animate-marquee flex w-max gap-12 whitespace-nowrap">
            {[...Array(2)].map((_, dup) => (
              <div key={dup} className="flex gap-12">
                {["AVA STUDIOS", "LUMEN LAB", "NORTHWIND", "HEXA PRINT", "ORBIT MEDIA", "VELA WAVES", "GLOW BEAUTY", "PEAK SUPPLY"].map((name) => (
                  <span key={name + dup} className="font-display text-sm font-bold tracking-[0.25em] text-slate-600">{name}</span>
                ))}
              </div>
            ))}
          </div>
              ...
              <Tilt max={6} className="h-full">
                <div className="glass card-glow group flex h-full flex-col rounded-3xl p-6">
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <Link href="/contact" className="text-xs font-medium text-slate-500 transition-colors hover:text-white">Get a quote →</Link>
                  </div>
                </div>
              </Tilt>
            </Reveal>
            ...
            {PROCESS.map(({ Icon, step, title, desc }, i) => (
              <Reveal key={step} delay={i * 90}>
                <div className="group relative h-full overflow-hidden rounded-3xl border border-white/8 bg-panel p-6 transition-colors hover:border-brand-400/40">
                  <span className="font-display absolute -right-2 -top-6 text-7xl font-bold text-white/4 transition-colors group-hover:text-brand-500/10">{step}</span>
      ...
      <section className="mx-auto max-w-7xl px-5 py-24 sm:px-8">
        <Reveal>
          <div className="mx-auto max-w-2xl text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-cyan-300">The reel</p>
            <h2 className="font-display mt-3 text-4xl font-bold text-white sm:text-5xl">Work you can <span className="text-gradient">feel</span></h2>
            <p className="mt-4 text-slate-400">A 3D carousel of recent cuts — hover to pause and click through.</p>
          </div>
        </Reveal>
        <div className="mt-10"><Reel3D items={reelItems} /></div>
        <div className="mt-6 text-center">
          <Link href="/work" className="inline-flex items-center gap-2 text-sm font-semibold text-cyan-300 transition-colors hover:text-white">Browse the full portfolio <ArrowRight size={15} /></Link>
        </div>
      </section>
          ...
          <div className="mx-auto max-w-7xl px-5 sm:px-8">
            <Reveal>
              <div className="mx-auto max-w-2xl text-center">
                ...
                <h2 className="font-display mt-3 text-4xl font-bold text-white sm:text-5xl">Rated by the people <span className="text-gradient">we edit for</span></h2>
              </div>
            </Reveal>
                ...
                <Reveal key={r.id} delay={i * 90}>
                  <Tilt max={5} className="h-full">
                    <figure className="glass flex h-full flex-col rounded-3xl p-6">
                      <Stars value={r.stars} />
                      <blockquote className="mt-4 flex-1 text-sm leading-relaxed text-slate-300">“{r.comment}”</blockquote>
                      <figcaption className="mt-5 flex items-center gap-3 border-t border-white/8 pt-4">
                        <div className="grid h-9 w-9 place-items-center rounded-full bg-gradient-to-br from-brand-600 to-cy-500 text-sm font-bold text-white">{r.clientName?.[0]?.toUpperCase()}</div>
                        <div>
          ...
          <Reveal>
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div>
                ...
                <h2 className="font-display mt-3 text-4xl font-bold text-white">Editing tips & <span className="text-gradient">insider notes</span></h2>
              </div>
              <Link href="/blog" className="text-sm font-semibold text-cyan-300 transition-colors hover:text-white">All articles →</Link>
            </div>
          </Reveal>
                ...
                <Link href={`/blog/${p.slug}`} className="group block h-full">
                  <div className="glass card-glow overflow-hidden rounded-3xl transition-transform">
                    <div className="relative h-44 overflow-hidden">
                      <img src={p.featuredImage} alt={p.title} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                    </div>
                    <div className="p-5">
                      <div className="flex items-center gap-2 text-[11px] text-slate-500">
                        <span className="font-semibold uppercase tracking-widest text-cyan-300">{p.categoryName || "Studio"}</span>
                        <span>·</span>
                        <span>{fmtDate(p.publishedAt)}</span>
                      </div>
                      <h3 className="font-display mt-2 text-lg font-semibold leading-snug text-white transition-colors group-hover:text-brand-300">{p.title}</h3>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-8 sm:px-8">
        <Reveal>
          <div className="bg-aurora relative overflow-hidden rounded-[2.5rem] border border-brand-400/20 p-10 text-center sm:p-16">
            ...
            <h2 className="font-display relative text-3xl font-bold text-white sm:text-5xl">Got footage? <span className="text-gradient">Let's make it move.</span></h2>
            <p className="relative mx-auto mt-4 max-w-xl text-slate-400">Send a brief today and get a plan, a timeline and a quote back within 24 hours. No calls required.</p>
            <div className="relative mt-8 flex flex-wrap justify-center gap-4">
              <Link href="/contact" className="glow-ring rounded-full bg-[#7357FF] px-8 py-4 text-sm font-black uppercase tracking-wider text-white transition-transform hover:scale-105 hover:bg-[#6346E8]">Book a Call</Link>
              <Link href="/portal" className="glass rounded-full border border-white/15 px-8 py-4 text-sm font-bold uppercase tracking-wider text-[#F6F3EC] transition-all hover:border-[#7357FF]/50 hover:text-white">Client Portal</Link>
            </div>
          </div>
        </Reveal>
      </section>
