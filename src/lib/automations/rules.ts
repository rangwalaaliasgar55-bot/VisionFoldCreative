 ...
 * no I/O — which is what lets the whole rulebook be tested against fixtures in
...
export type AttentionItem = {
  /** Stable across runs — this is what dedupes nudges. */
  key: string;
...
export function evaluate(snapshot: Snapshot): Evaluation {
  ...
  for (const lead of snapshot.leads) {
    ...
    items.push({
      ...
      detail: `Brief landed ${Math.floor(hours)}h ago — your promise on the site is a reply within ${cfg.leadSlaHours} hours.`,
      href: "/admin/leads",
      entityId: lead.id,
  ...
  for (const invoice of snapshot.invoices) {
    ...
    if (overdueDays > 0) {
      items.push({
        ...
        detail: `₹${money(invoice.amount)} outstanding.`,
        href: "/admin/invoices",
        entityId: invoice.id,
        ageDays: overdueDays,
      });
      ...
      if (canNudge(state, nudgeKey, now, cfg.nudgeCooldownHours)) {
        effects.push({
          ...
          nudgeKey,
          body: `Quick note: invoice ${invoice.number || `#${invoice.id}`} for ₹${money(invoice.amount)} is now ${overdueDays} day${overdueDays === 1 ? "" : "s"} past its due date. You can settle it from the Invoices tab here. If it's already been sent, ignore this and let us know.`,
        });
      }
  ...
  for (const project of snapshot.projects) {
    ...
    if (project.status === "in_review" && idleDays >= cfg.approvalNudgeDays) {
      items.push({
        ...
        title: `“${project.title}” has been awaiting approval ${idleDays}d`,
      ...
      if (canNudge(state, nudgeKey, now, cfg.nudgeCooldownHours)) {
        effects.push({
          ...
          nudgeKey,
          body: `Gentle nudge on “${project.title}” — the cut has been ready for your review for ${idleDays} days. Approve it or drop your notes in the portal and we'll turn them around. Happy to jump on the timeline together if that's easier.`,
        });
      }
      ...
      items.push({
        ...
        title: `“${project.title}” hasn't moved in ${idleDays}d`,
    ...
    if (due) {
      ...
      if (daysLeft >= 0 && daysLeft <= cfg.dueSoonDays && project.progress < 100) {
        items.push({
          ...
          title: `“${project.title}” is due in ${daysLeft}d at ${project.progress}%`,
        ...
        items.push({
          ...
          kind: "project_due",
          severity: "high",
          title: `“${project.title}” passed its due date ${-daysLeft}d ago`,
