 ...
 *
 * Pure module ΓÇö the portal form, the server validation and the tests all share
...
export const BRIEF_FIELDS: BriefField[] = [
  ...
  },
  {
    ...
    help: "A real date beats ΓÇ£ASAPΓÇ¥ ΓÇö it tells us how to schedule the passes.",
    type: "date",
    required: true,
  },
  {
    ...
    help: "Drive, Dropbox, Frame.io, WeTransfer ΓÇö any link we can download from.",
    type: "url",
    required: true,
    placeholder: "https://drive.google.com/ΓÇª",
  },
  {
    id: "runtime",
    label: "Target length",
    type: "select",
    required: true,
    options: ["Under 30 seconds", "30ΓÇô60 seconds", "1ΓÇô3 minutes", "3ΓÇô10 minutes", "Over 10 minutes"],
  },
  {
    id: "aspectRatios",
    label: "Which formats?",
    help: "Pick every place this will be posted ΓÇö reframing later costs a pass.",
    type: "multiselect",
    required: true,
...
export function validateBrief(answers: BriefAnswers, now = new Date()): BriefValidation {
  ...
  // Quality checks ΓÇö filled in, but not usable.
  ...
  if (!brandKit) {
    warnings.push("No brand kit ΓÇö we'll use our own type and colour choices.");
  }
  ...
  if (deadline) {
    ...
    if (Number.isNaN(date.getTime())) {
      ...
      else if (days <= 2) warnings.push("That's a rush turnaround ΓÇö we'll confirm we can hit it.");
    }
  }

  const ratios = answers.aspectRatios;
  if (Array.isArray(ratios) && ratios.length >= 3) {
    warnings.push("Three or more formats means an extra reframing pass ΓÇö worth confirming you need them all.");
  }

  if (!isFilled(answers.references)) {
    warnings.push("No references ΓÇö the first cut will be our read of the brief.");
  }
...
/** A readable brief for the studio side ΓÇö no JSON blobs in the project record. */
