...
A working backlog for VisionFold, written against the repo as it stands ΓÇö no
...
## A. Conversion ΓÇö turning visitors into briefs

| # | Item | Why | Effort |
| --- | --- | --- | --- |
| 1 | ~~Case-study pages `/work/[slug]`~~ **DONE** | Every project now has a shareable, indexed URL. | Γ£à |
...
| 6 | Delivery-date estimator | "Brief today ΓåÆ first cut by the 14th" is more persuasive than "3ΓÇô5 days". | S |
...
| 9 | ~~FAQ section with `FAQPage` schema~~ **DONE** | Nine real answers on the home page, with schema. | Γ£à |
...
| # | Item | Why | Effort |
| --- | --- | --- | --- |
| 16 | Brief ΓåÆ approach ΓåÆ result structure per case study | The story is what sells; the video is the proof. | M |
| 17 | Before/after grade slider per project | You already have the component ΓÇö give it real stills. | S |
...
| 21 | ~~`CreativeWork` + `VideoObject` structured data~~ **DONE** | Shipped with the case-study pages. | Γ£à |
| 22 | ~~Auto-generated OG image per project~~ **DONE** | Branded 1200├ù630 card generated per case study. | Γ£à |
| 23 | Filter by industry as well as format | Prospects self-identify by their sector, not by "16:9". | S |
| 24 | Password-protected private case studies | Some client work can't be public ΓÇö show it selectively. | M |
...
| 31 | ~~Auto-generated OG images for posts~~ **DONE** | Same treatment on the journal. | Γ£à |
...
| # | Item | Why | Effort |
| --- | --- | --- | --- |
| 39 | ~~Per-visit database load~~ **DONE (differently)** | Content queries are now tag-cached; full ISR is blocked by the session read in the layout. | Γ£à |
| 40 | ~~Tag-based cache invalidation on publish~~ **DONE** | Admin writes bust `portfolio` / `posts` tags immediately. | Γ£à |
...
| 53 | ~~Reel cards linking to their case study~~ **DONE** | The 3D reel and the work grid both link through. | Γ£à |
| 54 | Magnetic cursor on primary CTAs | Subtle pull toward the one action that matters. | S |
| 55 | Page-load "shutter" transition | Brand-appropriate ΓÇö you fold stories into motion. | M |
...
| 58 | ~~Reduced-data mode~~ **DONE** | Save-Data and 2G skip WebGL entirely; CSS atmosphere remains. | Γ£à |
...
| # | Item | Why | Effort |
| --- | --- | --- | --- |
| 77 | ~~Vercel cron schedule~~ **ALREADY EXISTED** | `vercel.json` runs the job daily at 06:00 UTC. | Γ£à |
...
| 80 | Proposal generator from a lead | Brief ΓåÆ formatted proposal ΓåÆ PDF. | L |
...
| 85 | Capacity planner ΓÇö hours committed vs available | Stops overbooking before it happens. | L |
...
| 90 | ~~CI running `npm run verify` on every PR~~ **DONE** | GitHub Actions: verify + build on push and PR. | Γ£à |
...
| 95 | ~~Audit log surfaced in the UI~~ **DONE** | `/admin/activity` with filtering and colour-coded verbs. | Γ£à |
...
| # | Item | Why | Effort |
| --- | --- | --- | --- |
| 97 | Conversion funnel analytics (visit ΓåÆ brief ΓåÆ client) | `visitors` is tracked; nothing answers "what works". | M |
| 98 | ~~UTM capture stored on the lead~~ **DONE** | Source, medium, campaign, referrer and landing page. | Γ£à |
...
1. ~~#1 case-study pages~~ Γ£à done
2. ~~#53 reel ΓåÆ case study~~ Γ£à done
3. ~~#39 per-visit database load~~ Γ£à done (tag-cached queries)
4. ~~#22 / #31 OG images~~ Γ£à done ┬╖ ~~#90 CI~~ Γ£à done ┬╖ ~~#98 UTM~~ Γ£à done
5. **#89 component tests** ΓÇö the last piece of the confidence story
...
**Cheapest wins:** #4, #5, #9, #10, #11, #45, #53, #58, #74, #98 ΓÇö all `S`, all
unblocked.
