...
Legend ΓÇö effort: `S` under an hour ┬╖ `M` a session ┬╖ `L` multiple sessions.

---

## Phase 0 ΓÇö Loose ends (do first, they're cheap)

| Item | Why | Effort |
| --- | --- | --- |
| **Cron schedule** ΓÇö `crons` entry in `vercel.json` + `CRON_SECRET` | The chase automations exist and work, but only when someone clicks *Run now*. Scheduling makes them actually autonomous. | S |
| **Kill the 19 lint warnings** ΓÇö `<img>` in admin/portal, two exhaustive-deps | They're the noise that hides a real warning later. Admin images can use `next/image` now that remote patterns are configured. | S |
| **`.env.example`** | Nobody can deploy this cleanly today without reading the source for env names. | S |

---

## Phase 1 ΓÇö The operational gaps (highest value in the whole document)

### 1.1 Transactional email ΓÇö **nothing emails anyone today** `M` ┬╖ needs a provider key
...
clear log when unset ΓÇö the same pattern `lib/ai.ts` already uses. Then wire:

- New lead ΓåÆ email you within seconds (with the brief and a reply-to)
- Lead auto-reply ΓåÆ "we've got it, expect a reply within 24 hours" (backs the
  promise the site makes)
- Invoice sent / overdue ΓåÆ email the client
- Portal message ΓåÆ email the recipient
- Weekly digest ΓåÆ what the automations flagged
...
result, related projects, `CreativeWork` structured data ΓÇö and add them to the
sitemap. Every project becomes a landing page and a sales asset.

---

## Phase 2 ΓÇö Performance & cost
...
| **Admin/portal images ΓåÆ `next/image`** | Media library and portal thumbnails are unoptimised; heaviest screens in the app. | S |
...
| **Font self-hosting** | Still one render-blocking round-trip to Google. `next/font` fixes it ΓÇö it just couldn't build in my sandbox. Worth doing where the network allows. | S |

---

## Phase 3 ΓÇö Frontend craft
...
| **View Transitions between routes** | Navigation is the last place that still feels like a website rather than one camera move. Progressive enhancement ΓÇö no cost where unsupported. | M |
| **Reel ΓåÆ case study** | The 3D reel currently links to `/work`. Point each card at its own case study and the whole home page becomes a funnel. | S |
...
| **Cursor-reactive grade** *(optional)* | Subtle colour-temperature shift following the pointer. Pure flourish ΓÇö last on the list, first to cut. | S |

---

## Phase 4 ΓÇö Confidence in the codebase

| Item | Why | Effort |
| --- | --- | --- |
| **Component tests** (Vitest + Testing Library) | Five pure-logic gates exist and they've caught real bugs. Nothing tests a *component* ΓÇö the reel, the compare slider and the intake form are all untested UI. | M |
...
## Phase 5 ΓÇö Bigger product bets
...
| **AI assist inside the Social Studio** | The engine is deterministic and offline by design. An optional AI pass ΓÇö using the existing `lib/ai.ts` providers ΓÇö could rewrite a chosen draft while keeping the offline path as the default. |
...
2. **1.2 rate limiting** ΓÇö no blockers, closes a real hole
3. **1.3 case-study pages** ΓÇö biggest SEO and sales gain available
4. **1.1 email** ΓÇö the moment you have a Resend key; the highest operational value of anything here
5. **Phase 2 ISR** ΓÇö speed and database cost
6. Then Phase 3 craft, Phase 4 confidence, Phase 5 by appetite

## What I need from you

- **Resend account** (or SMTP) ΓåÆ unlocks 1.1
- **Sentry account** ΓåÆ unlocks monitoring
- A **real reel or client video** ΓåÆ unlocks the hero video option
- A **pricing decision** ΓåÆ unlocks the packages page
